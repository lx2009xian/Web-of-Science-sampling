...
df=pd.read_csv("D:/2020国自然/1-医学或生理学—样本及对照抽样数据/diversity未删除参考文献/2021.3.3data updata/number/number.csv")
...
for index,url in enumerate(df['total_url']):
        ...
        if np.ceil((sample - 4) / 10) +1 == np.ceil((sample + 5) / 10):
            ...
            page_text = bro.page_source
            tree = etree.HTML(page_text)
            for i in range(int((sample - 4)-(value-1)*10),11):
            ...
            tree = etree.HTML(page_text)
            # for i in range(1, 11):
            for i in range(int((sample - 4) - (value - 1) * 10), int(sample + 5 + 1 - (value - 1) * 10)):
                for div in tree.xpath('//div[@class="search-results"]'):
                    links.append(
                        "https://apps.webofknowledge.com/" + str(div.xpath('./div/div[3]/div/div/div/a/@href')[i - 1]))
                    titles.append(div.xpath('./div/div[3]/div/div/div/a/value/text()')[i - 1])
        sample_titles.append(titles)
        sample_links.append(links)
pd.DataFrame(sample_titles).to_csv("D:/2020国自然/1-医学或生理学—样本及对照抽样数据/diversity未删除参考文献/2021.3.3data updata/number/title.csv")
pd.DataFrame(sample_links).to_csv("D:/2020国自然/1-医学或生理学—样本及对照抽样数据/diversity未删除参考文献/2021.3.3data updata/number/link.csv")
